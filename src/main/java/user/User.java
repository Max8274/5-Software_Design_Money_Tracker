...
public class User
{
    ...
    public void addInUserHashmap(UUID paidUser, Double price)
    {
        ...
        if (this.userHashmap.containsKey(paidUser) &&
                RegistrationDB.getUserDatabase().getValueDBHashmap(paidUser).get(0).getUserHashmap().containsKey(this.ID))
        {
            Double priceForPaidUser = this.userHashmap.get(paidUser);
            Double priceForThisUser = RegistrationDB.getUserDatabase().getValueDBHashmap(paidUser).get(0).getUserHashmap().get(this.ID);

            if (priceForPaidUser > priceForThisUser)
            {
                this.userHashmap.put(paidUser, priceForPaidUser - priceForThisUser);
                RegistrationDB.getUserDatabase().getValueDBHashmap(paidUser).get(0).getUserHashmap().remove(this.ID);
            }
            ...
            {
                this.userHashmap.remove(paidUser);
                RegistrationDB.getUserDatabase().getValueDBHashmap(paidUser).get(0).getUserHashmap().put(this.ID, priceForThisUser - priceForPaidUser);
            }
        }
    }

    public void clearUserHashMap()
    {
    ...
    public String toPay()
    {
        ...
        {
            text.append("  To ").append(RegistrationDB.getUserDatabase().getValueDBHashmap(paidUser).get(0).getName()).append(": ").append(this.userHashmap.get(paidUser)).append(" euro");
        }
